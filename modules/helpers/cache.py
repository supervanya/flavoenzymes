...
def cached_reqest(
    baseurl, params=None, auth=None, headers=None, useCaching=USE_CACHING
):
...
def brenda_cached_reqest(request_name, params, request_fn, useCaching=USE_CACHING):
    unique_ident = params_unique_combination(
        baseurl=request_name, params={p: p for p in params[2:]}
    )
    ...
    # if not, fetch it
    log(
        f"Making a request for new data...\n{request_name}, params: {json.dumps(params)}",
        "debug",
    )
...
def get_smile_string(compound_id, useCaching=USE_CACHING):
    ...
    unique_ident = params_unique_combination(
        baseurl="smile_string", params={"cpd": compound_id}
    )
    ...
    if ChEBI_id:
        ...
        # if not, fetch it
        log(
            f"Making a request for new data...\ncompound_request, cpd: {compound_id}",
            "debug",
        )
        ...
        try:
            ...
            raise Exception(
                f"ChEBI_id: {ChEBI_id} does not have a SMILES string. Original Error {e}"
            )
